...
SELECT 
    upper(i.INSTANCE_NAME ||'.'||vd.name) || '|' || 'sys_time_model' || '|' || S.STAT_NAME || '|' || ROUND(s.value/1000000) AS result
FROM 
    v$instance i
JOIN 
    v$con_sys_time_model s ON s.stat_name IN ('DB time', 'DB CPU')
JOIN 
    v$containers vd ON vd.con_id = s.con_id
JOIN 
    v$database d ON d.cdb = 'YES'
WHERE 
    vd.con_id <> 2
...
FROM
    ...
    v$pgastat p
WHERE
    p.name IN ('total PGA allocated', 'process count');

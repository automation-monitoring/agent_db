-- This huge query has been stolen from Checkmk's mk_postgres, who in turn had
-- who in turn had "gratefully taken [it] from Greg Sabino Mullane's check_postgres.pl"
SELECT current_database() AS db, schemaname, tablename, reltuples::bigint
  ...
  AS wastedbytes, CASE WHEN relpages < otta THEN '0 bytes'::text
  ELSE (bs*(relpages-otta))::bigint || ' bytes' END
  ...
  END AS wastedipages, CASE WHEN ipages < iotta THEN 0
  ELSE bs*(ipages-iotta) END AS wastedibytes,
  CASE WHEN ipages < iotta THEN '0 bytes'
  ELSE (bs*(ipages-iotta))::bigint || ' bytes' END AS wastedisize,
  ...
  AS iotta FROM pg_class cc JOIN pg_namespace nn ON cc.relnamespace = nn.oid
